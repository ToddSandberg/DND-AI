...
## Saved Messages
The chat log is written to `./web-server/messages.json` every time a message is sent, edited, or generated by the DM, and is read back when the server starts, so the story survives a restart.

To start a fresh game, stop the server and delete `./web-server/messages.json`. It will be recreated with the opening message on the next start.
